...
os.chdir("../visualizations")  # changing directory to save visualizations
...
plt.plot(test_data, color = 'blue', label='Actual House Price')
plt.plot(fc_series, color = 'orange',label='Predicted House Price')
plt.fill_between(lower_series.index, lower_series, upper_series, 
                 color='k', alpha=.10)
plt.title(config["model_data"]["forecast"]["title"], fontsize = 14)
plt.xlabel(config["model_data"]["forecast"]["xlabel"])
plt.ylabel(config["model_data"]["forecast"]["ylabel"])
